...
def update_telemetry(n_clicks, year, gp, session_type, selected_drivers):
    ...
    try:
        ...
        # ── GAP traces (row 6) — ref = fastest driver ─────────────
        ...
        fig.add_hline(
            y=0, line=dict(color=_BORDER, width=1), row=6, col=1
        )
        ...
        # Repete a legenda para cada gráfico (centralizada)
        legend_dict = dict(
            orientation="h",
            yanchor="bottom",
            xanchor="center",
        ...
        for row_idx in range(1, 7):
            fig.update_xaxes(showlegend=False, row=row_idx, col=1)
        ...
        return fig, pilot_cards, track_map_fig, _visible, None, dominance_items
