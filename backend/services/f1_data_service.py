...
3. F1DataService -> DataRepository (business logic + in-memory cache)
...
_available_cache: dict = {"data": None, "ts": 0.0}
_track_layout_cache: dict = {}   # key: (season, event, session_type)
...
class F1DataService:
    ...
    def get_season_info(self, season: int) -> Optional[Dict[str, Any]]:
        ...
        if season not in self.repo.get_all_seasons():
            return None
    ...
    def get_session_results(
        self, season: int, event: str, session_type: str = "R"
    ) -> List[Dict[str, Any]]:
        ...
        if not session_data:
            raise Exception("Session not found")
        return self.repo.get_results_for_session(
    ...
    def get_drivers_in_session(
        ...
        if not session_data:
            return []
        return self.repo.get_drivers_in_session(
            season, session_data["id"]
        )
    ...
    def get_driver_telemetry(
        ...
        if not event_data:
            raise Exception(
                f"Event '{event}' not found for season {season}"
            )
        session_data = self.repo.get_session(
            season, event_data["id"], session_type
        )
    ...
    def get_track_layout(
        ...
        Cached for process lifetime — layout never changes for a given
        season/event/session combination.
        """
        cache_key = (season, event, session_type)
        if cache_key in _track_layout_cache:
            return _track_layout_cache[cache_key]
        ...
        result = self.repo.get_track_layout_samples(
            season, session_data["id"]
        )
        if result.get("count", 0) > 0:
            _track_layout_cache[cache_key] = result
        return result
    ...
    def get_available_data(self) -> Dict[str, Any]:
        ...
        Cached for _CACHE_TTL seconds to avoid repeated DB scans on every
        dropdown interaction.
        """
        now = time.time()
        if (
            _available_cache["data"] is not None
            and now - _available_cache["ts"] < _CACHE_TTL
        ):
            return _available_cache["data"]
        data = self.repo.get_available_data()
        _available_cache["data"] = data
        _available_cache["ts"] = now
        return data

    def get_available_sessions_for_event(
        self, season: int, event: str
    ) -> List[str]:
        """Get list of available session types for an event."""
        event_data = self.repo.get_event_by_name(season, event)
        if not event_data:
            return []
        sessions = self.repo.get_sessions_for_event(
            season, event_data["id"]
        )
